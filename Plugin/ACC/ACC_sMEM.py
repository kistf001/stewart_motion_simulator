...
def demo():
    #import struct
    #afddf = struct.unpack('!fff', info.physics.accG)

    global fdfd

    while(400):
        #print(info.static.track, info.graphics.tyreCompound, info.graphics.currentTime,
        #      info.physics.rpms, info.graphics.currentTime, info.static.maxRpm, 
        #      list(info.physics.tyreWear))
        
        #print(
        #    "ACC : ", info.physics.accG[0], info.physics.accG[1], info.physics.accG[2], 
        #    "VEL : ", info.physics.velocity[0], info.physics.velocity[1], info.physics.velocity[2], 
        #    info.physics.pitch, info.physics.roll)

        fdfd[0] = info.physics.localAngularVel[0]
        fdfd[1] = info.physics.localAngularVel[1]
        fdfd[2] = info.physics.localAngularVel[2]

        print(info.physics.localAngularVel[2])
        time.sleep(0.01)
    #fdfd[0] = info.physics.localAngularVel[0]
    #fdfd[1] = info.physics.localAngularVel[1]
    #fdfd[2] = info.physics.localAngularVel[2]
